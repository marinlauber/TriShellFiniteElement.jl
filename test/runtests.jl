...
@testset "TriShellFiniteElement.jl" begin
    ...
    # default Ferrite.jl node ordering
    ...
    # Scaled triangle: P1=(2,0,0), P2=(0,3,0), P3=(0,0,0), area=3 TODO default ordering of nodes in Ferrite.jl
    x_scaled = [Vec{3}((2.0, 0.0, 0.0)), Vec{3}((0.0, 3.0, 0.0)), Vec{3}((0.0, 0.0, 0.0))]
    reinit!(scv, x_scaled)
    ...
    reinit!(scv, x_unit)
    @test scv.∇N[1, 1] ≈ Vec{2}(( 1.0,  0.0))
    @test scv.∇N[1, 2] ≈ Vec{2}(( 0.0,  1.0))
    @test scv.∇N[1, 3] ≈ Vec{2}((-1.0, -1.0))
end
